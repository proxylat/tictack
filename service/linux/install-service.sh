...
sudo mkdir -p "$DEPLOY"
sudo cp "$DIR"/TicTackSv "$DIR"/TicTackSv.dll "$DIR"/*.deps.json "$DIR"/*.runtimeconfig.json "$DEPLOY/"
sudo cp "$DIR"/*.dll "$DIR"/*.so "$DEPLOY/"
sudo cp "$DIR/config.yaml" "$DEPLOY/"
sudo chmod +x "$DEPLOY/TicTackSv"
